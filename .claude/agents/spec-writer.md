...
5. Define acceptance criteria as testable assertions. Avoid prose like "works correctly." Write things a test can check: "calling `loadEntity(path)` returns the parsed Entity within 10ms when the file is in Store FS," "deleting `~/.claude/work/X/spec.md` and re-running produces an identical spec." When an acceptance criterion will be enforced by a CI-style content guard (e.g. "the bespoke `FooService` class is gone from frontend/src" or "no test asserts exhaustive Redux state shape"), write the regex narrowly — `/toEqual.*\bstate\b|Object\.keys\([^)]*\bstate\b/` is the right precedent for exhaustive-state guards; broad alternatives like `/getState\(\)/` alone catch benign typed reads and create noise. (Calibration finding from #155 batch.)
