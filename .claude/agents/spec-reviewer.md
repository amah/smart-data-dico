...
- **Framework citations**: open every `.d.ts` cited. Confirm the type/function exists with the claimed signature. A miss here is a hard reject — agents downstream will build code that doesn't compile.
- **Acceptance criteria**: are they testable? "Works correctly" is not testable. "All existing tests pass" is acceptable but insufficient on its own.
