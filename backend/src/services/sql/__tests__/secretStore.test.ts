...
describe('secretKey', () => {
  it('is stable across connection-field ordering and prefixed by package', () => {
    const a = secretKey('orders', 'postgres', { host: 'h', port: 5432, database: 'o' }, 'app');
    const b = secretKey('orders', 'postgres', { database: 'o', port: 5432, host: 'h' }, 'app');
    expect(a).toBe(b);
    expect(a.startsWith('orders::')).toBe(true);
  });

  it('differs by user, connection, and package', () => {
    const base = secretKey('orders', 'postgres', { host: 'h' }, 'app');
    expect(secretKey('orders', 'postgres', { host: 'h' }, 'other')).not.toBe(base);
    expect(secretKey('orders', 'postgres', { host: 'other' }, 'app')).not.toBe(base);
    expect(secretKey('billing', 'postgres', { host: 'h' }, 'app')).not.toBe(base);
  });
});

describe('AES-GCM round-trip', () => {
  it('saves and retrieves a password', async () => {
    const k = secretKey('orders', 'postgres', { host: 'h' }, 'app');
  ...
  it('returns null for a missing key', async () => {
    expect(await getSecret(secretKey('orders', 'postgres', { host: 'h' }, 'nobody'))).toBeNull();
  });
...
describe('deletion', () => {
  ...
  it('deleteSecretsForPackage removes only that package', async () => {
    const ordersA = secretKey('orders', 'postgres', { host: 'h' }, 'app');
    const ordersB = secretKey('orders', 'postgres', { host: 'h2' }, 'app');
    const billing = secretKey('billing', 'postgres', { host: 'h' }, 'app');
    await saveSecret(ordersA, 'a');
    await saveSecret(ordersB, 'b');
    await saveSecret(billing, 'c');

    await deleteSecretsForPackage('orders');
