...
- **Services** (`src/services/`): Business logic — `serviceService.ts` (entities, search, impact; plus `setEntityHidden`/`setEntityStyle` for the view features), `dictionaryService.ts` (packages), `stereotypeService.ts`, `perspectiveService.ts` (BFS resolution), `importService.ts`, `exportService.ts`, `qualityService.ts`, `visibilityService.ts` (hide policy: `system.hidden` flag + `hideRules[]`, #hide-model-data). All domain services consume `IStorageBackend`. **AI authoring guidance** is centralised in `services/ai/authoringRules.ts` (`AUTHORING_RULES`) — the single source of truth the integrated agent injects into its system prompt (designer mode, both provider paths); `docs/SKILL.md`/`docs/format-reference.md` mirror it (guarded by a sync test). `fileOperations.ts` (slice 5) and `schemaEntityWriter.ts` (slice 5b) are fully migrated. An ESLint `no-restricted-imports` rule on `fs` and `fs/promises` blocks new direct-`fs` imports outside an allow-list (slice 5c; see `backend/.eslintrc.cjs`). Three legacy production sites — `controllers/modelMetadataController.ts`, `routes/project.routes.ts`, `services/dicoConfigService.ts` — are temporarily allow-listed pending follow-up migrations. Bootstrap, storage internals, scripts, and tests are permanently allow-listed.
- **SQL run** (`src/services/sql/`): Executes AI-generated queries against a package's physical database, read-only. `sqlGuards.ts` hard-enforces a single SELECT/WITH (`assertReadOnlySelect`, comment/literal-stripping so `;` and DML keywords inside strings are ignored). `connectionCache.ts` holds DB credentials **in memory only, per package, with a sliding TTL (~30 min) — never persisted, never logged, password redacted from every response** (`redact()`). `executors.ts` opens a server-side cursor per dialect (pg `DECLARE CURSOR`/`FETCH FORWARD` in a `READ ONLY` tx, oracle `resultSet.getRows`, mysql/mssql streams wrapped by `bufferedRowCursor.ts`, sqlite via Node's built-in `node:sqlite` buffered into chunks) so the grid fetches chunks on demand without re-running the query; `resultRegistry.ts` tracks open cursors with TTL + LRU eviction. `sqlRunService.ts` ties it together (connect/run/fetchMore/close). Passwords can be *saved* via `secretStore.ts` (#209: Electron safeStorage → keytar → `DICO_SECRET_KEY` AES-GCM; refuses plaintext) and connection *parameters* via `connectionLibrary.ts` — a global per-app-user named library in `~/.dico-app/dico-app.json` (`GET/POST/PUT/DELETE /api/sql/connections`, secret-strip at any depth, passwords only in the secret store under `conn::<id>::<userTag>`); `POST /api/sql/connect` accepts a `connectionId` and resolves the password server-side. The pg/mysql2/mssql/oracledb drivers are **optional peer dependencies**, loaded lazily (`loadDriver`, with fallback resolution from the launch dir / global npm root / `DICO_DRIVER_PATH` for npx runs) with an actionable "npm install <driver>" error when absent; SQLite uses the built-in `node:sqlite` (Node ≥22.5 `--experimental-sqlite`, which the CLI enables automatically). Postgres and SQLite are verified against real databases; mysql/mssql/oracle still need live verification.
...
- **ai-assistance** — Chat panel, conversation history, prompt CRUD, slash commands; consumes data-dictionary services for grounding. Fenced ```sql blocks get a **▶ Run** button → `SqlRunModal` (read-only connect form prefilled from the package physical config, chunked results grid that fetches more on scroll, and on a syntax/DB error surfaces the failed query + error into the chat thread via an `ai-chat:sql-error` event, so the agent explains it and replies with a corrected query). API client: `sqlRunApi` in `services/api.ts`. The agent authors all concepts (createEntity/…/createStateMachine) and, via the agent-tool registry, plugin tools (reverse-engineer, element-style). It injects `AUTHORING_RULES` (the shared format contract) into its system prompt in designer mode. Tool-call ordering: the OpenAI-compatible direct client (`aiDirectClient.ts`) emits only tool events during the loop and the controller streams the final reply **once, after** them — so the answer never streams before its tool calls (matching the Anthropic/Vercel path).
...
### View features — Hide (#hide-model-data) & Element Style (#element-style)

Two non-destructive *view* layers, both configured in `dico.config.json` and resolved once at read/build time (never per-render):
- **Hide** — declutter reverse-engineering waste. `system.hidden` metadata (`"false"` pins visible) or `hideRules[]` (glob/regex on table/entity/package). Enforced by `visibilityService` behind `getVisibleServiceEntities`/`GET …/entities?includeHidden=`; `getServiceEntities` stays full so graphs keep the complete set. Endpoints `GET/PUT /api/config/hide-rules`, `PUT …/entities/:entity/hidden`. Docs: `hide-model-data.md`.
- **Element Style** — style elements by role in diagrams + a list badge. Named `elementStyles[]` (theme-token colors) + `styleRules[]` (match stereotype/role/name → style); the frontend resolver (`utils/elementStyle.ts`) picks a `styleName` (override → rule → detected role → stereotype; zero-tagging detectors for junction/reference/remote-ref), applied via generated `node[styleName="…"]` selectors (`cytoscapeStylesheet.ts`) after an `applyElementStyles` post-pass. `/element-styles` page; `GET/PUT /api/config/element-styles|style-rules`, `PUT …/entities/:entity/style`. Docs: `element-style.md`.

Both contribute **AI agent tools** via the agent-tool registry (`services/ai/agentToolRegistry.ts`, no `aiController` edits): `setEntityHidden` is a service method; element-style registers `defineElementStyle`/`addStyleRule`/`setEntityStyle` (`services/elementStyle/agentTools.ts`).
