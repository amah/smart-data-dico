...
### Fixed
- **AI replies appearing only after completion.** OpenAI-compatible providers
  now stream their real response deltas through the backend to the chat panel,
  with proxy buffering disabled and split SSE records preserved across network
  reads. Completed replies are not replayed after their streamed text.
